...
# value = [A321neo,A330-900neo,A330-900]
    ...
    def planning(self, sx, sy, gx, gy):
        ...
        while 1:
            ...
            if current.x == goal_node.x and current.y == goal_node.y:
                ...
                for i in range(len(cost)):
                    if num[i] > fmax:
                        print(A[i],"not viable!")
                        cost[i] = math.inf
                    else:
                        cost[i] = (current.cost * F[i] * Cf + current.cost * Ct[i] + Cc[i]) * num[i]
                        print(A[i],"->",cost[i])
                select = cost.index(min(cost))
                print("Solution ->", A[select], "*", num[select])
