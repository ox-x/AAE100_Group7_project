    ...
    def planning(self, sx, sy, gx, gy):
        ...
        while 1:
            ...
            if show_animation:  # pragma: no cover
                ...
                if len(closed_set.keys()) % 20000 == 0:
                    plt.pause(0.001)
...
def main():
    ...
    for i in range(20,30): # draw the free border
        ox.append(i)
        oy.append(2*i - 40)
    ...
    for i in range(-12, 60): 
        for j in range(best_lc_position, best_lc_position + 5):
            lc_x.append(i)
            lc_y.append(j)  

    a_star = AStarPlanner(ox, oy, grid_size, robot_radius, fc_x, fc_y, tc_x, tc_y, lc_x, lc_y) # initialize the A star planner
    rx, ry = a_star.planning(sx, sy, gx, gy)
    ...
    cost_a350 = a_star.calculate_cost(plane_names[2], planes['A350-900'], a350_flights,level)
    costs = {
        'A321neo': cost_a321,
